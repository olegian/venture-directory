...
fn draw_directory(stdout: &mut Stdout, dir: &Directory, is_current: bool, config: &Config) -> (usize, usize) {
    ...
    something like {startidx, endidx, currentidx}, that indexes into some structure. Could reuse Directory struct, just need to add
    these slicing things, and then change iterators to start from start and end at end
